...
namespace VFS.PMS.ApplicationPages.Layouts.VFSProjectH2
{
    public partial class HRDecisions : LayoutsPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                LogHandler.LogError(ex, "Error in PMS HR Decisions");
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    ...
                    using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                    {
                        using (SPWeb web = osite.OpenWeb())
                        {
                            ...
                            // CurrentListItem["appAppraisalStatus"] = "H2 – Completed";
                            ...
                            SPListItem lstStatusItem = appraisalStatus.GetItemById(20);
                            ...
                            //ht["tskStatus"] = "Close";       //Convert.ToString(lstStatusItem["Appraisal_x0020_Workflow_x0020_S"]);
                            //ht["Status"] = "Close";
                            ...
                            q.Query = "<Where><And><Eq><FieldRef Name='aphAppraisalId' /><Value Type='Number'>" + Convert.ToInt32(hfAppraisalID.Value) + "</Value></Eq><Eq><FieldRef Name='aphAppraisalPhase' /><Value Type='Text'>H2</Value></Eq></And></Where></Query>";
                        ...
                        //Context.Response.Write("<script type='text/javascript'>alert('" + string.Format(CustomMessages.Completed, "H2") + "');window.open('" + url + "','_self'); </script>");
                        Context.Response.Write("<script type='text/javascript'>window.open('" + CommonMaster.DashBoardUrl + "','_self'); alert('" + string.Format(CustomMessages.Completed, "H2") + "');</script>");
                    }
                });
                WorkflowTriggering("Close");
            }
                ...
                if (!string.IsNullOrEmpty(appLog.Value))
                {
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        ...
                        using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                        {
                            ...
                            {
                                web.AllowUnsafeUpdates = true;
                                //SPList listAppPhase = web.Lists["Appraisal Phases"];
                                SPList list = web.Lists["Appraisals"];
                                SPListItem CurrentListItem = list.GetItemById(Convert.ToInt32(hfAppraisalID.Value));
                                // SPListItem appPhaseListItem = listAppPhase.GetItemById(Convert.ToInt32(hfAppraisalPhaseID.Value));
                                SPList appraisalStatus = web.Lists["Appraisal Status"];

                                SPListItem lstStatusItem = appraisalStatus.GetItemById(22);
                                ...
                                SPListItem lstItem;
                                SPQuery q = new SPQuery();
                                q.Query = "<Where><And><Eq><FieldRef Name='aphAppraisalId' /><Value Type='Number'>" + hfAppraisalID.Value + "</Value></Eq><Eq><FieldRef Name='aphAppraisalPhase' /><Value Type='Text'>H2</Value></Eq></And></Where>";
                                SPListItemCollection coll = lstAppraisalPhases.GetItems(q);
                            ...
                            Context.Response.Write("<script type='text/javascript'>alert('" + string.Format(CustomMessages.Review, "H2") + "');window.open('" + CommonMaster.DashBoardUrl + "','_self'); </script>");
                        }
                    });
                    WorkflowTriggering("Request");
                }
