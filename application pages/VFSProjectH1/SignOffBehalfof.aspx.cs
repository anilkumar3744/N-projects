...
{
    public partial class SignOffBehalfof : LayoutsPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                LogHandler.LogError(ex, "Error in PMS Sign-Off On Behalf of Appraisee PageLoad");
        ...
        protected void BtnSignoff_Click(object sender, EventArgs e)
        {
            ...
            {
                LogHandler.LogError(ex, "Error in PMS Sign-Off On Behalf of Appraisee SignOffButton");
                Context.Response.Write("<script type='text/javascript'> " + CommonMaster.serializeMessage(ex.Message) + ";</script>");
            }
        }
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate()
            {
                using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                {
                    using (SPWeb web = osite.OpenWeb())
                    {
                        ...
                        q.Query = "<Where><And><Eq><FieldRef Name='AssignedTo' /><Value Type='User'>" + hfAppraisee.Value + "</Value></Eq><And><Eq><FieldRef Name='tskStatus' /><Value Type='Text'>" + lblStatusValue.Text + "</Value></Eq><Eq><FieldRef Name='tskAppraisalId' /><Value Type='Number'>" + Convert.ToInt32(hfAppraisalID.Value) + "</Value></Eq></And></And></Where>";
                        SPListItemCollection taskCollection = lstAppraisalTasks.GetItems(q);
                        if (taskCollection.Count > 0)
                        {
                            SPListItem taskItem = taskCollection[0];
                            Hashtable ht = new Hashtable();
                            ht["tskStatus"] = "Sign Off";
                            ht["Status"] = "Sign Off";
                            ht["tskActionTakenBy"] = this.currentUser;
                            web.AllowUnsafeUpdates = true;
