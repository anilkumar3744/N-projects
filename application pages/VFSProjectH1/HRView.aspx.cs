    ...
    public partial class HRView : LayoutsPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //    lblStatusValue.Text = "H1-Completed";
                //}
                ...
                if (!this.IsPostBack)
                {
                    ...
                    using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                    {
                        using (SPWeb currentWeb = osite.OpenWeb())
                        {
                            ...
                            if (!CommonMaster.CanUserViewAppraisal(Convert.ToDouble(appraisalItem["appEmployeeCode"]), this.currentUser.LoginName, currentWeb))
                            {
                            ...
                            {
                                Context.Response.Write("<script type='text/javascript'>alert('The appraisal has no H1 details');window.frameElement.commitPopup();</script>");
