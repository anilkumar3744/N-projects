    ...
    public partial class ReviewerEvaluation : LayoutsPageBase
    {
                ...
                if (!this.IsPostBack)
                {
                    ...
                    using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                    {
                        using (SPWeb currentWeb = osite.OpenWeb())
                        {
                            ...
                            //if (phaseItem["aphIsAppealed"] != null)
                            //    lblReValue.Text = "(Appeal)";
                            //if (Convert.ToString(taskItem["appIsReview"]) == "True")
                            //{
                            //    lblReValue.Text = "(H1-HR Review)";
                            //}
                            //else if (phaseItem["aphIsAppealed"] != null)
                            //{
                            //    lblReValue.Text = "(Appeal)";
                            //}
                            ...
                            else if (Convert.ToString(taskItem["appIsReview"]) == "True")
                            {
                                lblReValue.Text = "(H1-HR Review)";
                            }
        ...
        protected void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in RptRevaluation.Items)
            {
                ...
                {
                    dr["SNo"] = (item.FindControl("lblSno1") as Label).Text;
            ...
            //dr1["agGoal"] = (item.FindControl("lblScore") as Label).Text;
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    ...
                    string url = SPContext.Current.Web.Url + "/_layouts/VFSProjectH1/ReviewerEvaluation.aspx?AppId=" + hfAppraisalID.Value;
        ...
        protected void BtnApprove_Click(object sender, EventArgs e)
        {
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    ...
                    using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                    {
                        using (SPWeb web = osite.OpenWeb())
                        {
                            ...
                            SPListItem lstStatusItem = appraisalStatus.GetItemById(7);
                        ...
                        Context.Response.Write("<script type='text/javascript'>alert('Approved successfully');window.open('" + CommonMaster.DashBoardUrl + "','_self'); </script>");
                        //Context.Response.Write("<script type='text/javascript'> " + CommonMaster.serializeMessage(ex.Message) + ";</script>"); 
                    }
                    //}
                    //else
            ...
            {
                LogHandler.LogError(ex, "Error in PMS Reviewer Evaluation Approve");
                Context.Response.Write("<script type='text/javascript'> " + CommonMaster.serializeMessage(ex.Message) + ";</script>");
            }
        ...
        protected void LnkDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Label lblSno1 = rptItem.FindControl("lblSno1") as Label;
                //this.dummyTable = ViewState["Appraisals"] as DataTable;
                DataTable dt = ViewState["Appraisals"] as DataTable;
                ...
                foreach (RepeaterItem item in RptRevaluation.Items)
                {
                    ...
                    if (!string.IsNullOrEmpty(lblScore1.Text) && (i != Convert.ToInt32(lblSno1.Text) - 1))
                    {
                ...
                dt.Rows.RemoveAt(Convert.ToInt32(lblSno1.Text) - 1);
                i = 0;
            ...
            {
                LogHandler.LogError(ex, "Error in PMS H1 Reviewer Evaluation");
        ...
        protected void txtWeightage_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txtWeightage.Text))
                weitage = Convert.ToDecimal(txtWeightage.Text.Trim());

            if (!string.IsNullOrEmpty(txtEvaluation.Text))
                evaluation = Convert.ToDecimal(txtEvaluation.Text.Trim());
            ...
            lblScoretotal.Text = Convert.ToString(total);//CommonMaster.GetRating(total);
            //decimal finalValue = Convert.ToDecimal(lblScoretotal.Text);//Upendra 
            ...
            using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
            {
                using (SPWeb objWeb = osite.OpenWeb())
                {
                    ...
                    foreach (RepeaterItem item in RptRevaluation.Items)
                    {
                        ...
                        SPQuery q = new SPQuery();
                        q.Query = "<Where><And><Eq><FieldRef Name='aphAppraisalId' /><Value Type='Number'>" + Convert.ToString(hfAppraisalID.Value) + "</Value></Eq><Eq><FieldRef Name='aphAppraisalPhase' /><Value Type='Text'>H1</Value></Eq></And></Where>";
                        SPListItemCollection coll = lstAppraisalPhases.GetItems(q);

                        lstItem = coll[0];

                        lstItem["aphScore"] = Convert.ToString(lblScoretotal.Text);
            ...
            using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
            {
                using (SPWeb objWeb = osite.OpenWeb())
                {
                    ...
                    foreach (RepeaterItem item in rptCompetencies.Items)
                    {
                        ...
                        //lstItem["agAppraisalId"] = Convert.ToInt32(hfAppraisalID.Value);
                        //lstItem["agAppraisalPhaseId"] = Convert.ToInt32(hfAppraisalPhaseID.Value);
                        //lstItem["acmptAppraisalPhase"] = Convert.ToString(lblAppraiser.Text);
