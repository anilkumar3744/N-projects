...
namespace VFS.PMS.ApplicationPages.Layouts.VFSProjectH1
{
    public partial class SignOff : LayoutsPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                if (!this.IsPostBack)
                {
                    ...
                    using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                    {
                        using (SPWeb currentWeb = osite.OpenWeb())
                        {
                            ...
                            //string appealbtn = Convert.ToString(phaseItem["aphIsAppealed"]);
                            //if (appealbtn == "Appealed")
                            //{
            ...
            {
                LogHandler.LogError(ex, "Error in PMS Sign-Off PageLoad");
        ...
        protected void BtnSignoff_Click(object sender, EventArgs e)
        {
                    ...
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        ...
                        using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                        {
                            using (SPWeb web = osite.OpenWeb())
                            {
                                ...
                                SPListItem lstStatusItem = appraisalStatus.GetItemById(8);
                            ...
                            //Context.Response.Write("<script type='text/javascript'>alert('Signed Off successfully');window.open('" + url + "','_self'); </script>");
                            Context.Response.Write("<script type='text/javascript'>alert('" + string.Format(CustomMessages.SignOff, "H1") + "');window.open('" + CommonMaster.DashBoardUrl + "','_self'); </script>");
                        }
                    });
                    WorkflowTriggering("Sign Off");
            ...
            {
                LogHandler.LogError(ex, "Error in PMS Sign-Off SignOffButton");
                Context.Response.Write("<script type='text/javascript'> " + CommonMaster.serializeMessage(ex.Message) + ";</script>");
            }
        }
        protected void BtnAppeal_Click(object sender, EventArgs e)
        {
                    ...
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        ...
                        using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                        {
                            using (SPWeb web = osite.OpenWeb())
                            {
                                ...
                                SPListItem lstStatusItem = appraisalStatus.GetItemById(10);
                            ...
                            //Context.Response.Write("<script type='text/javascript'>alert('Appealed successfully');window.open('" + url + "','_self'); </script>");
                            Context.Response.Write("<script type='text/javascript'>alert('" + string.Format(CustomMessages.Appeal, "H1") + "');window.open('" + CommonMaster.DashBoardUrl + "','_self'); </script>");
                        }
                    });
                    WorkflowTriggering("Appeal");
                }
            ...
            {
                LogHandler.LogError(ex, "Error in PMS Sign-Off AppealButton");
                Context.Response.Write("<script type='text/javascript'> " + CommonMaster.serializeMessage(ex.Message) + ";</script>");
            }
        }
        ...
        public void SaveComments(int appraisalID, int appraisalPhaseId)
        {
            using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
            {
                using (SPWeb objWeb = osite.OpenWeb())
                {
                    ...
                    SPListItem lstItem;
                    SPQuery q = new SPQuery();
                    q.Query = "<Where><And><Eq><FieldRef Name='aphAppraisalId' /><Value Type='Number'>" + appraisalID + "</Value></Eq><Eq><FieldRef Name='aphAppraisalPhase' /><Value Type='Text'>H1</Value></Eq></And></Where>";
                    SPListItemCollection coll = lstAppraisalPhases.GetItems(q);
