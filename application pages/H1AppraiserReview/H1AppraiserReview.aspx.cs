...
namespace VFS.PMS.ApplicationPages.Layouts.H1AppraiserReview
{
    public partial class H1AppraiserReview : LayoutsPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (TabContainer1.ActiveTabIndex == 3)
                {
                    ...
                    Upadepanel.Triggers.Add(trigger);
                }
                if (!this.IsPostBack)
                {
                    SPUser appraisee;
                    using (SPSite osite = new SPSite(SPContext.Current.Web.Url))
                    {
                        using (SPWeb currentWeb = osite.OpenWeb())
                        {
                            ...
                            //phasesQuery.Query = "<Where><Eq><FieldRef Name='aphAppraisalId' /><Value Type='Number'>" + Convert.ToInt32(hfAppraisalID.Value) + "</Value></Eq><Eq><FieldRef Name='ID' /><Value Type='Counter'>" + Convert.ToInt32(pipAppraisalPhaseID.Value) + "</Value></Eq></Where>";
                            ...
                            hfAppraisalPhaseID.Value = phaseItem["ID"].ToString();
                            if (phaseItem["aphHRReviewLatestComments"] != null)
                            {
                                lblHRReviewFinalComments.Text = phaseItem["aphHRReviewLatestComments"].ToString();
                            }
                            ...
                            if (spItem != null)
                            {
                                ...
                                {
                                    if (phaseItem["aphIsPIP"] != null && phaseItem["aphIsPIP"].ToString() == "True")
                                    {
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    ...
                    // SaveGoals(Convert.ToInt32(Request.Params["ID"]),0);
                    //SaveGoals(Convert.ToInt32(hfAppraisalID.Value), Convert.ToInt32(hfAppraisalPhaseID.Value));
                    //SaveCompetencies(Convert.ToInt32(Request.Params["ID"]));
                    ...
                    // SavePDP(Convert.ToInt32(Request.Params["ID"]));
                    ...
                    string url = SPContext.Current.Web.Url + "/_layouts/H1AppraiserReview/H1AppraiserReview.aspx?AppId=" + hfAppraisalID.Value;
        ...
        public void SaveItem(bool NewItem, int ItemID)
        {
            ...
            //ClientScript.RegisterStartupScript(GetType(), "test", CommonMaster.serializeMessage(sss)); 
            //}
            //catch (Exception ex)
            //{
            //    Page.ClientScript.RegisterClientScriptBlock(typeof(SPAlert), "alert", "<script language=\"javascript\">alert('" + ex.Message + " .')</script>");
